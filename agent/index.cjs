// Persistent interval log to confirm server stays alive
setInterval(() => {
  console.log(`[heartbeat] Server alive at ${new Date().toISOString()}`);
}, 5000);
// Global error handlers to prevent server exit
...
const warmSession = require('./warm_session');
...
const jobs = {}; // In-memory job map

app.post('/api/release-snipe', (req, res) => {
  const runId = crypto.randomUUID();
  jobs[runId] = { status: 'pending', started: Date.now(), result: null, error: null };
  res.status(202).json({ runId }); // Respond immediately

  setImmediate(async () => {
    try {
      // ...existing booking/sniper logic...
      // On success:
      jobs[runId].status = 'completed';
      jobs[runId].result = {/* ...result data... */};
    } catch (err) {
      jobs[runId].status = 'failed';
      jobs[runId].error = err.message || String(err);
    }
  });
});
app.get('/api/release-snipe/:runId', (req, res) => {
  const job = jobs[req.params.runId];
  if (!job) return res.status(404).json({ error: 'runId not found' });
  res.json(job);
});
...
// Graceful shutdown
let server = null;
// Graceful shutdown logic disabled for persistent server operation
    ...
    const slots = await page.$$eval('a[href*="/bookings/book/"]', (anchors) => {
      ...
      const guessBookedCount = (row) => {
        ...
        const nameTokens = tokens.filter((t) => {
          ...
          if (
            /book|booking|available|waiting|wait list|open|holes|format|course|tee|sheet|buggy|buggies|price|rate|member|login|book now|member booked/i.test(
              lower,
            )
          )
            return false;
...
server = app.listen(PORT, HOST, (err) => {
  ...
  console.log(`✅ fairway-sniper-agent listening on http://${HOST}:${PORT}`);
  console.log(`   - Health: http://${HOST}:${PORT}/api/health`);
  console.log(`   - Sniper (unified): POST http://${HOST}:${PORT}/api/snipe`);
  console.log(`   - Fetch Tee Times: POST http://${HOST}:${PORT}/api/fetch-tee-times`);
  console.log(`   - Release Snipe: POST http://${HOST}:${PORT}/api/release-snipe`);
  console.log(`   - Release Snipe Status: GET http://${HOST}:${PORT}/api/release-snipe/:runId`);
});
server.on('error', (err) => {
