...
async function tryBookTime(page, time, players = []) {
  ...
  for (let i = 0; i < players.length && i < 3; i++) {
    ...
    if ((await select.count()) > 0) {
      await select.selectOption({ label: playerName }).catch((err) => {
        console.warn(
          `Could not select "${playerName}" in player ${i + 2}:`,
          err.message,
        );
      });
    }
  ...
  for (const sel of confirmSelectors) {
    ...
    if (await btn.isVisible().catch(() => false)) {
      ...
      console.log(
        `✅ Clicked confirm for ${time} with players: ${players.join(', ')}`,
      );
      return true;
    }
  }
...
async function testBooking() {
  ...
  const confirmBtn = page
    .locator('button:has-text("Confirm"), button:has-text("Book Now")')
    .first();
